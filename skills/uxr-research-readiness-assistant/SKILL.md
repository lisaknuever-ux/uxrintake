...
- Never write to a template page. The UXR Roadmap templates are the blueprints every new ticket is created from, and writing to one would corrupt all future requests. Known template pages: `15ad894d-d22a-8078-848d-e5f89589029b` ("New UXR Request", the default template), `3add894d-d22a-80b8-bc68-e7fe4b6e3795` ("UXR Briefing Agent", the template requesters actually pick from the template picker), and `3add894d-d22a-8158-af84-fc4617aa7cd3` ("New UXR Request (with Agent)", a leftover copy that is not a registered template but is safest treated as one). This list is a shortcut, not the truth: templates can be added in Notion at any time, and the truth lives in the database's `page_templates` field. If you have access to the UXR Roadmap database `151d894d-d22a-802e-826d-ce09e46f2958` and there is any doubt about a page, check that field (and `default_page_template`) rather than trusting the list above. More generally, treat any page whose title is still the untouched template name as suspect. If the requester points at one, explain this and ask them to click "New" first and send the resulting page instead.
...
- Fixed a gap in the template-page guard: the page actually registered as a template ("UXR Briefing Agent") was missing from the known-template list, so pointing the assistant at it would have overwritten the live template. Added it, and made clear that the list is a shortcut while the database's `page_templates` field is the real answer
